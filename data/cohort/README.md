...
`raw_sampled_works.json` and `cohort.csv` (and the same files under `pi_proxy/`) are author-level derivatives of OpenAlex records and are not redistributed in the public repository; `reproduce.sh` re-extracts them from the OpenAlex API when they are absent. `transition_rates.csv` and `author_origin_overrides.csv` are included.
