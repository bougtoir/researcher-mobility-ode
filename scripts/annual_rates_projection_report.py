...
def plot_annual_rates(rate_table, projected_rates, fig_dir=None):
    ...
    fig.suptitle("Observed (solid) and projected (dashed) transition rates by research macro-region, 2000-2026", fontsize=12)
    plt.tight_layout()
...
def plot_interciv_heatmap(flows, fig_dir=None):
    ...
    ax.set_title("Total cross-region abroad author-years by origin (rows) and destination (columns), 2000-2023", fontsize=11)
...
def plot_projection_by_compartment(projected, observed, fig_dir=None):
    ...
    fig.suptitle("Observed (solid) and projected (dashed) compartment counts by research macro-region, 2017-2023", fontsize=12)
