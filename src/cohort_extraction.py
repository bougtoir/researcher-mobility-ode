...
# PI proxy definitions.  ``first_last``: first last-author paper (baseline);
# ``corresponding``: first paper flagged is_corresponding by OpenAlex;
# ``recurrent_last``: second last-author paper (last author on >= 2 papers).
#
# FIELD-SPECIFIC: these proxies assume the AI/ML (computer science) authorship
# convention in which the group leader is listed last (or is the corresponding
# author) and single-author papers are independent work.  The hit proxy
# (is_in_top_10_percent within HIT_WINDOW years) is likewise scored within
# the AI/ML subfield.  In fields with alphabetical author order (mathematics,
# economics, high-energy physics), first-author-PI conventions or
# consortium authorship, edit ``is_last_author`` / ``pi_years_by_definition`` /
# ``is_top10`` (and SUBFIELD / --subfield-id) before reusing this code.
PI_DEFINITIONS = ("first_last", "corresponding", "recurrent_last")
PI_DEFINITION = "first_last"
...
def is_corresponding_author(work, target_aid):
    for auth in work.get("authorships", []):
        aid = author_id_key((auth.get("author") or {}).get("id"))
        if aid == target_aid:
            return bool(auth.get("is_corresponding"))
    return False


def pi_years_by_definition(works, aid):
    """Return {definition: first year the author qualifies as PI, or None}.

    ``works`` must be sorted by publication year.
    """
    first_last = None
    recurrent_last = None
    corresponding = None
    ...
    for w in works:
        ...
        if is_last_author(w, aid):
            ...
            if first_last is None:
                first_last = y
            if n_last == 2 and recurrent_last is None:
                recurrent_last = y
        if corresponding is None and is_corresponding_author(w, aid):
            corresponding = y
    return {
        "first_last": first_last,
        "corresponding": corresponding,
        "recurrent_last": recurrent_last,
    }
...
def classify_author(aid, works, a2g, origin_override=None, pi_definition=None):
    """Return a dict with career descriptors for one author.

    ``pi_definition`` selects which PI proxy populates ``pi_year``/``pi``
    (default: module-level ``PI_DEFINITION``); the first-qualifying year under
    every definition in ``PI_DEFINITIONS`` is also returned so alternative
    proxies can be evaluated without re-extraction.
    ...
    # PI proxy (single-authored papers count as last-author)
    pi_definition = pi_definition or PI_DEFINITION
    if pi_definition not in PI_DEFINITIONS:
        raise ValueError(f"Unknown PI definition: {pi_definition}")
    pi_years = pi_years_by_definition(works, aid)
    pi_year = pi_years[pi_definition]
    ...
    return {
        ...
        "pi_year_first_last": pi_years["first_last"],
        "pi_year_corresponding": pi_years["corresponding"],
        "pi_year_recurrent_last": pi_years["recurrent_last"],
...
def main():
    ...
    globals()["PI_DEFINITION"] = args.pi_definition
