...
def setup_db(db_path):
    ...
    # Normalised authorship table so PI proxies (last / corresponding /
    # recurrent last author) can be re-derived with SQL without re-fetching.
    conn.execute(
        """
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_as_author ON authorships(author_id)")
    return conn
...
# Bump whenever the stored work payload or the authorships table changes
# (v2 added is_corresponding and the authorships table).  A cache written
# under an older version cannot be reused for PI-proxy classification.
SCHEMA_VERSION = 2
...
def check_cache_schema(conn, state, db_path, state_path):
    """Refuse to reuse a cache written by an older extraction schema."""
    ...
    stale = (n_works > 0 or state.get("partitions")) and (
        version != SCHEMA_VERSION or (n_works > 0 and n_auth == 0)
    )
    if stale:
        raise SystemExit(
            f"ERROR: {db_path} / {state_path} were written by extraction schema "
            f"{version!r} (works={n_works}, authorships={n_auth}); current schema is "
            f"{SCHEMA_VERSION} (is_corresponding + authorships table). Delete both files "
            "and re-run to re-fetch, or point --db-path/--state-path elsewhere."
        )
...
def _flush(conn, batch_works, batch_authors, batch_authorships=None):
    ...
    if batch_authorships:
        conn.executemany(
            ...
            "(work_id, author_id, year, author_position, is_corresponding, n_authors, country_codes) "
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            batch_authorships,
        )
    conn.commit()
...
def _process_page(w, a2g):
    ...
    authorship_rows = []
    seen = set()
    n_authors = len(reduced["authorships"])
    for auth in reduced["authorships"]:
        aid = author_id_key(auth["author"].get("id"))
        if aid and aid not in seen:
            seen.add(aid)
            author_rows.append((aid, wid))
            codes = ",".join(sorted({i["country_code"] for i in auth["institutions"] if i.get("country_code")}))
            authorship_rows.append((
                wid, aid, year, auth.get("author_position"),
                int(auth.get("is_corresponding", False)), n_authors, codes,
            ))
    groups = work_groups(reduced, a2g)
    return works_row, author_rows, authorship_rows, reduced, groups
    ...
    try:
        while not stop_event.is_set():
            ...
            for w in results:
                ...
                if out is None:
                    continue
                works_row, authors, authorship_rows, reduced, groups = out
                works_batch.append(works_row)
                authors_batch.extend(authors)
                authorships_batch.extend(authorship_rows)
            ...
            q.put((
                ...
                next_cursor, pages_done, works_done, author_rows_done, authorships_batch,
            ))
            if not next_cursor:
                break
            cursor = next_cursor
...
def _writer(q, conn, state, state_path, pages_per_commit, n_partitions, a2g, sample_per_group):
    ...
    while sentinels < n_partitions:
        ...
        # Error signal
        if item[1] == "ERROR":
            _flush(conn, batch_works, batch_authors, batch_authorships)
            save_state(state_path, state)
            raise OpenAlexBudgetExhausted(item[2])

        (_, works_batch, authors_batch, reduced_batch, groups_batch, next_cursor,
         pages_done, works_done, author_rows_done, authorships_batch) = item
        batch_works.extend(works_batch)
        batch_authors.extend(authors_batch)
        batch_authorships.extend(authorships_batch)
    ...
    if batch_works or batch_authors or batch_authorships:
        _flush(conn, batch_works, batch_authors, batch_authorships)
        save_state(state_path, state)
...
def fetch_and_store(client, conn, a2g, state, state_path, subfield_id, start_year,
    ...
    total_works = sum(p.get("works", 0) for p in partitions.values())
    total_authors = sum(p.get("author_rows", 0) for p in partitions.values())
    # The writer thread saves the reservoir sample to state_path + '.sampled.json'.
    sampled_path = Path(str(state_path) + ".sampled.json")
...
def classify_all_authors(conn, a2g, min_works, overrides, batch_size=500, pi_definition=PI_DEFINITION):
    ...
    ce.PI_DEFINITION = pi_definition
...
def main():
    ...
    parser.add_argument("--pi-definition", choices=PI_DEFINITIONS, default=PI_DEFINITION,
                        help="PI proxy used for the pi/pi_year columns and transition rates. "
                             "All three candidate PI years are always written to cohort.csv.")
    ...
    check_cache_schema(conn, state, db_path, state_path)
    sampled_works = []
    ...
    cohort = classify_all_authors(
        ...
        pi_definition=args.pi_definition,
    )
